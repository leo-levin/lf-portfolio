...
const HOMEPAGE_SCROLL_THRESHOLD_DESKTOP = 600
...
function App() {
  ...
  const manuallyExpandedRef = useRef(false) // Track if cloud was manually expanded
  ...
  const handleWhiteDotClick = () => {
    if (dotCloudRef.current) {
      if (dotCloudRef.current.isExpanded) {
        isCollapsingRef.current = true
        // Don't reset manuallyExpanded here - keep it true to prevent auto-collapse
        dotCloudRef.current.collapseCloud()
        setIsCloudExpanded(false)
      } else {
        isCollapsingRef.current = false
    ...
    <>
      ...
      <GridItem col={isMobile ? 1 : rightColumn+1} row={isMobile ? bottomRow+8 : 9} colSpan={isMobile ? 5 : 3} align="top-left">
        <div style={{
          ...
          <a href="mailto:leolfrankel@gmail.com" className="resume-link">leolfrankel@gmail.com</a><br />
          <a href="tel:+13104632774" className="resume-link">310 463 2774</a><br />
          <br />
      ...
      {/* Spacer to ensure page extends below last project */}
      <div style={{
        height: `${Number(bottomRow) * 9 * 80 + (isMobile ? MOBILE_PROJECT_OFFSET_ROWS * 80 : 0) + 800}px`,
        position: 'relative',
        pointerEvents: 'none'
      }} />
    </>
